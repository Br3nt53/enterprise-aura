...
DEFAULT_EXCLUDES = {
    ...
    ".ruff_cache", ".idea", ".vscode", "node_modules", ".DS_Store"
}
...
def render_tree(root: Path, exclude: set[str]) -> list[str]:
    ...
    def walk(dir: Path, prefix: str):
        entries = sorted([p for p in dir.iterdir() if not should_skip(p, exclude)], key=lambda p: (p.is_file(), p.name.lower()))
...
def to_json(root: Path, exclude: set[str]) -> dict:
    def build(d: Path) -> dict:
        children = []
        for p in sorted([p for p in d.iterdir() if not should_skip(p, exclude)], key=lambda p: (p.is_file(), p.name.lower())):
            if p.is_dir():
...
def main():
    ...
    ap.add_argument("--exclude", action="append", default=[], help="Extra names to exclude (repeatable)")
