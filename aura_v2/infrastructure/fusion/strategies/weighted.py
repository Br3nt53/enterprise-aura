...
def fuse(tracks_by_modality: Dict[str, List[Dict[str, Any]]], weights: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    for i in range(L):
        ...
        for k in keys:
            ...
            wsum += w
        if wsum > 0:
            fused.append({"bbox": [v / wsum for v in accum], "score": 1.0, "meta": {"fused_from": keys}})
    return fused
