...
class SystemReportGenerator:
    ...
    def generate_health_report(self, telemetry_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            ...
            "components": {
                "tracker": {"status": "active", "tracks": len(telemetry_data.get("active_tracks", []))},
                "sensors": {"status": "active", "count": telemetry_data.get("sensor_count", 0)},
    ...
    def generate_tracking_summary(self, tracking_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_processing_time = sum(result.get("processing_time_ms", 0) for result in tracking_results) / len(tracking_results)
        ...
        summary = {
            ...
            "performance_metrics": {
                "min_processing_time": min(result.get("processing_time_ms", 0) for result in tracking_results),
                "max_processing_time": max(result.get("processing_time_ms", 0) for result in tracking_results),
                "frames_with_tracks": sum(1 for result in tracking_results if result.get("active_tracks"))
            }
        }
        
        # Save summary
        summary_file = self.output_dir / f"tracking_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
